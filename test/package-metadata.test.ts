...
test('package.json exposes the TypeScript/ESM runtime surface', async () => {
  ...
  assert.equal(pkg.scripts['test:ts'], 'node --import tsx --test test/*.test.ts test/**/*.test.ts');
  assert.equal(pkg.scripts['start:dist'], 'node dist/src/index.js');
  assert.equal(pkg.scripts['test:dist'], 'node --test dist/test/*.test.js dist/test/**/*.test.js');
